...
适用场景：研发要把妙搭应用的**源码拉到本地**，用自己熟悉的本地 code agent（cc / Codex / Kimi Code）+ **原生 git** 开发调试，再部署到云端。应用类型 `fullstack_app`（React + Nest + Drizzle + PostgreSQL）。

## 核心心智（再强调一遍，这里最容易错）

- **代码读写一律走原生 `git`。** CLI 只做 `+git-credential-init`（配凭证）和 `+publish`（部署）两件事，clone/pull/push/diff/log 全用原生 git。
- **`develop` 是工作分支，`main` 是部署态只读快照。** 平时只在 `develop` 上 push；`main` 只能由 `+publish` 推进。
...
```
+create(fullstack_app, 勾选开启多环境数据库) → +git-credential-init → git clone
   → npm install && npm dev run（起本地开发，自动拉 env，非阻塞）
   → [按需用 +db-* 调库] → 本地编码 → git push miaoda develop
...
| 步骤 | 命令 | 说明 |
|------|------|------|
| 1. 新建应用 | `lark-cli apps +create --app-type fullstack_app --name "<名字>" --enable-multi-env-db`（flag 待定） | 服务端拉脚手架 + 初始化 git + 接好妙搭远端。**推荐建时就开启多环境数据库**，后续可直接本地调库。从响应拿 `app_id` 和 git 仓库地址 |
...
| 7. 推送 | `git push miaoda develop`（用原生 git） | 推到工作分支 develop；服务端 pre-receive hook 会校验，并触发后台预构建 |
| 8. 部署 | `lark-cli apps +publish --app-id app_xxx` | 发布当前 develop；成功后服务端自动 `main ← develop` fast-forward。**dev 的库结构变更也随这一步发到 online** |
...
- **remote 叫 `miaoda`，工作分支是 `develop`**：推送 `git push miaoda develop`，同步云端 `git fetch miaoda develop && git rebase miaoda/develop develop`。
- **服务端护栏**（CLI / 原生 git 都生效）：禁直推 `main`、拒 force-push、`develop` 必须 fast-forward。所以 push 被拒多半是云端 develop 比本地新 → 先 rebase 再 push。
...
**dev / online 双环境**（与代码的 develop / main 平行）：日常在 **dev** 调，`+db-*` 用 `--env` 选环境；**dev 的库结构变更会随 `+publish` 一起发布到线上 online**（和代码同一条发布链路，不需要单独的库发布动作）。所以在 dev 改完 schema 别忘了 `+publish` 才会上线。
